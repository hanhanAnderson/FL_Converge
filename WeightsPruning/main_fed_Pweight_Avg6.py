...
def get_sub_paras2(w_glob, wmask, bmask):
...
get_sub_paras2 before train for subnetwork

get_sub_paras after training for network without biases(low)
        ...
        for iter in range(args.epochs):
            ...
            for id, idx in enumerate(idxs_users):
                ...
                if typep == 1:
                    ...
                    w = get_sub_paras2(w, local_w_masks[1], local_b_masks[1])
                elif typep == 3:
                    type_array.append(3)
                    w, loss = local.train(net=copy.deepcopy(net_3).to(args.device))
                    w = get_sub_paras2(w, local_w_masks[2], local_b_masks[2])
                elif typep == 4:
                    type_array.append(4)
                    w, loss = local.train(net=copy.deepcopy(net_4).to(args.device))
                    w = get_sub_paras2(w, local_w_masks[3], local_b_masks[3])


                elif typep == 5:
                    type_array.append(5)
                    w, loss = local.train(net=copy.deepcopy(net_51).to(args.device))
                    w = get_sub_paras2(w, local_w_masks[4], local_b_masks[4])
                elif typep == 6:
                    type_array.append(6)
                    w, loss = local.train(net=copy.deepcopy(net_51).to(args.device))
                    w = get_sub_paras2(w, local_w_masks[5], local_b_masks[5])
                elif typep == 7:
                    type_array.append(7)
                    w, loss = local.train(net=copy.deepcopy(net_51).to(args.device))
                    w = get_sub_paras2(w, local_w_masks[6], local_b_masks[6])
                # if args.all_clients:
